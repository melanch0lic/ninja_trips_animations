...
class CachedLottie {
  ...
  CachedLottie(this.size, this.composition) : images = List.filled(composition.durationFrames.ceil(), null);
  ...
  Image _takeImage(BuildContext context, double progress) {
    ...
    return picture.toImageSync((size.width * devicePixelRatio).round(), (size.height * devicePixelRatio).round());
  }
}
...
class _CachedLottiePlayerState extends State<CachedLottiePlayer> with TickerProviderStateMixin {
  late final AnimationController _autoController = AnimationController(vsync: this, duration: widget.lottie.duration)
    ..repeat();
