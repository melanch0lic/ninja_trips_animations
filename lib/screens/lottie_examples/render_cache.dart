  ...
  Widget build(BuildContext context) {
    return ListView(
      children: [
        ...
        for (var align in [Alignment.bottomCenter, Alignment.center, Alignment.topRight])
          _Row(
            builder: (cache) {
              return Lottie.asset('assets/lottiefiles/bomb.json', renderCache: cache, height: 40, alignment: align);
            },
          ),
      ],
    );
...
class _Row extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          for (var cache in [null, RenderCache.raster, RenderCache.drawingCommands]) Expanded(child: builder(cache))
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return ListView(
      children: [
        ...
        Text('Memory: ${(RenderCache.raster.store.totalMemory / 1000000).toStringAsFixed(1)}MB'),
        const Divider(),
      ],
    );
  }
